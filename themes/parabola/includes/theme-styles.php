...
function parabola_register_styles() {
	...
	if($parabola_mobile=="Enable") {	wp_register_style( 'parabola-mobile', get_template_directory_uri() . '/style-mobile.css' );}
...
add_action('wp_print_styles', 'parabola_enqueue_styles' );
...
add_action('wp_head', 'parabola_styles_echo');
...
foreach ($parabolas as $key => $value) {
	...
	if ($parabola_mobile=="Enable") {
		...
		echo "<link rel='stylesheet' id='parabola_style_mobile'  href='".get_template_directory_uri() . '/style-mobile.css' . "' type='text/css' media='all' />";
	}
}

add_action ('wp_head','parabola_load_mobile_css');

// JS loading and hook into wp_enque_scripts
add_action('wp_head', 'parabola_customjs' );
...
add_action('wp_enqueue_scripts', 'parabola_scripts_method');
?>